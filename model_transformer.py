...
class EncoderLayer(nn.Module):
    ...
    def forward(self, src, src_mask):
        ...
        src = self.layer_norm(src + self.dropout(_src))
        #src = [barch size, src len, hid dim]
        return src
...
class Encoder(nn.Module):
    ...
    def __init__(self, input_dim, hid_dim, n_layers, n_heads,
        ...
        self.dropout = nn.Dropout(dropout)
        self.scala = torch.sqrt(torch.FloatTensor([hid_dim])).to(device)
        self.device = device
    ...
    def forward(self, src):
        ...
        src = self.dropout((self.tok_embedding(src) * self.scala) + self.pos_embedding(pos))
        #src = [batch size, src len, hid dim]
...
class MultiHeadAttentionLayer(nn.Module):
    ...
    def forward(self, query, key, value, mask = None):
        ...
        Q = Q.view(batch_size, -1, self.n_headsm self.head_dim).permute(0, 2, 1, 3)
        K = K.view(batch_size, -1, self.n_headsm self.head_dim).permute(0, 2, 1, 3)
        V = V.view(batch_size, -1, self.n_headsm self.head_dim).permute(0, 2, 1, 3)
